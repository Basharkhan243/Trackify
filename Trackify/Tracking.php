...
<body class="min-h-screen">
    ...
    const maxSessionTime = 3600; // 1 hour = 3600 seconds
    ...
    function updateTimer() {
        ...
        if (sessionSeconds >= maxSessionTime) {
            stopDueToSessionLimit();
        }
    }
    ...
    stopBtn.addEventListener('click', () => {
        forceStopTimer("Manual Stop");
    });

    function stopDueToSessionLimit() {
        forceStopTimer("1 hour limit reached! Please submit your work before continuing.");
    }

    function forceStopTimer(message) {
        clearInterval(timer);
        isRunning = false;
    ...
    // Mutation observer for scrolling on form show
    ...
    let idleTime = 0;
    const idleLimit = 3600; // 1 hours for idle detection (can be set lower for testing)
    ...
    function sendIdleLog() {
        if (isRunning) {
            ...
            timerDisplay.textContent = "Paused (Idle)";
            alert("You've been idle. Timer has been stopped.");
        }

        fetch("api/mark_idle.php", {
            method: "POST",
          ...
          .then(data => console.log("Idle logged:", data))
          .catch(err => console.error("Idle log error:", err));
    }
</script>
